-- Add slug column to contents table
ALTER TABLE public.contents ADD COLUMN slug text;

-- Create index for slug for performance
CREATE INDEX idx_contents_slug ON public.contents(slug);

-- Function to generate URL-friendly slug from title
CREATE OR REPLACE FUNCTION public.generate_slug(title text)
RETURNS text AS $$
BEGIN
  RETURN lower(
    regexp_replace(
      regexp_replace(
        regexp_replace(title, '[^a-zA-Z0-9\s-]', '', 'g'),
        '\s+', '-', 'g'
      ),
      '-+', '-', 'g'
    )
  );
END;
$$ LANGUAGE plpgsql;

-- Function to ensure unique slug
CREATE OR REPLACE FUNCTION public.ensure_unique_slug(title text, content_id text DEFAULT NULL)
RETURNS text AS $$
DECLARE
  base_slug text;
  final_slug text;
  counter integer := 0;
BEGIN
  base_slug := generate_slug(title);
  final_slug := base_slug;
  
  -- Check if slug already exists (excluding current content if updating)
  WHILE EXISTS (
    SELECT 1 FROM public.contents 
    WHERE slug = final_slug 
    AND (content_id IS NULL OR id != content_id)
  ) LOOP
    counter := counter + 1;
    final_slug := base_slug || '-' || counter;
  END LOOP;
  
  RETURN final_slug;
END;
$$ LANGUAGE plpgsql;

-- Update existing contents with slugs
UPDATE public.contents 
SET slug = ensure_unique_slug(title, id)
WHERE slug IS NULL;

-- Trigger to automatically generate slug for new content
CREATE OR REPLACE FUNCTION public.generate_content_slug()
RETURNS TRIGGER AS $$
BEGIN
  -- Only generate slug if it's not provided or empty
  IF NEW.slug IS NULL OR NEW.slug = '' THEN
    NEW.slug := ensure_unique_slug(NEW.title, NEW.id);
  END IF;
  
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Create trigger for INSERT
CREATE TRIGGER trigger_generate_content_slug_insert
  BEFORE INSERT ON public.contents
  FOR EACH ROW
  EXECUTE FUNCTION public.generate_content_slug();

-- Create trigger for UPDATE (when title changes)
CREATE TRIGGER trigger_generate_content_slug_update
  BEFORE UPDATE ON public.contents
  FOR EACH ROW
  WHEN (OLD.title IS DISTINCT FROM NEW.title AND (NEW.slug IS NULL OR NEW.slug = ''))
  EXECUTE FUNCTION public.generate_content_slug();